...
/** kturgut Login Page Actions */

function redirect_login_page()
{
	$loginPage = home_url("auth");
...
{
	$loginPage = home_url("auth?actionType=isEmpty");
...
{
	$loginPage = home_url("auth?actionType=isErrora");

	if ($error->has_errors()) {
		if (!empty($error->get_error_message("loginCaptchaError"))) {
			$_SESSION["login_errors"] =  [$error->get_error_message("loginCaptchaError")];
			$loginPage = home_url("auth?actionType=captchaError");
		}
	}

	$_SESSION["login_errors"] =  $error->get_error_messages();
...
function login_form_add_recaptcha()
{
	//Output your HTML
	$html_txt = '<p>
	<div class="g-recaptcha w-100" data-sitekey="6LeNq1IhAAAAAJI5O931lkPDWRmJK4zcfXuyHCMt"></div>
	</p>
...
add_filter('login_form_middle', 'login_form_add_recaptcha');

function g_recaptcha_verify($user_token, $secret_key = "6LeNq1IhAAAAACXyhvQesbv3CjaNu2MI3Z2KmcrA")
{
	$remote_ip = $_SERVER["REMOTE_ADDR"];
	$captcha = file_get_contents("https://www.google.com/recaptcha/api/siteverify?secret=$secret_key&response=$user_token&remoteip=$remote_ip");
...
function wdm_validate_login_captcha($user, $password)
{
	...
	if (!g_recaptcha_verify($userToken)) {
		$return_value = new WP_Error('loginCaptchaError', 'Captcha doğrulanamadı, lütfen tekrar deneyiniz.');
		return $return_value;
	}
	return $user;
}
